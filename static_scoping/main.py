...
def variables_declaration(line: str, current_scope: Scope, line_index: int):
...
def new_scope(line: str, current_scope: Scope, scopes: dict[ScopeId, Scope]) -> None | Scope:
...
def end_of_scope(line: str, current_scope: Scope) -> bool:
    """
...
def print_scope(scope: Scope):
    var_str = ", ".join([str(var) for var in scope.variables])
    print(f"Scope ({scope}) variables:", var_str)
...
def main():
    ...
    for i in range(len(lines)):
        line = lines[i]
        variables_declaration(line, current_scope, i)
        potential_new_scope = new_scope(line, current_scope, scopes)
        potential_end_of_scope = end_of_scope(line, current_scope)
        
        if potential_new_scope is not None:
            # add new scope to the current scope
            current_scope = potential_new_scope
            scopes[current_scope.id] = current_scope
